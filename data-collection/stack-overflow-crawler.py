...
MONGO_PORT = 27017
MONGO_DATABASE = "clone_detection"
...
def main():
    ...
    for site in SITES:
        ...
        tag_info = requests.get("https://api.stackexchange.com/2.3/tags/solidity/info?key="+STACK_EXCHANGE_API_KEY+"&site="+site[1]).json()
        ...
        for page in tqdm(range(1, int(math.ceil(int(tag_info["items"][0]["count"]) / 100)) + 1)):
            search_result = requests.get("https://api.stackexchange.com/2.3/search?key="+STACK_EXCHANGE_API_KEY+"&page="+str(page)+"&pagesize=100&order=asc&sort=creation&tagged=solidity&site="+site[1]).json()
