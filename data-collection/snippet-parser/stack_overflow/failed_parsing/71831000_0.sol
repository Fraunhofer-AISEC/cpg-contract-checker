...
struct User { address addr; uint256 cycle; address upline; uint256 referrals; uint256 payouts; uint256 direct_bonus; uint256 match_bonus; uint256 deposit_amount; uint256 deposit_payouts; uint40 deposit_time; uint256 total_deposits; uint256 total_structure; }
...
uint256 public subscriptionFee = 100 * (10**3);
...
constructor(
    ...
    ref_bonuses.push(50 * (10**decimals));
    ref_bonuses.push(50 * (10**decimals));
    ref_bonuses.push(50 * (10**decimals));
...
function join(address _upline) external {
    ...
    require(
        (users[_upline].deposit_time > 0) || (_upline == address(0)),
        "upline not exist"
    );
    ...
    transferToken(usdt, wallet1, spends);
...
function contractInfo()
    external
    view
    returns (uint256 _total_users, uint256 _total_deposited)
{`
    return (total_users, total_deposited);
}


fallback() external payable { require(count < 3); count++; } }
