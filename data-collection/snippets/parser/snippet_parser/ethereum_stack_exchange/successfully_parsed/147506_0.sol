...
contract Payment {
    ...
    function isPaid(address walletAddress) public view returns(uint){
        return SlotTenants[walletAddress].due;
    }

    function isActive(address walletAddress) public view returns(bool){
        return SlotTenants[walletAddress].active;
    }
    function getCarPlate(address walletAddress) public view returns(string memory){
        return SlotTenants[walletAddress].CarPlate;
    }
    ...
    function checkInSlot(address walletAddress,uint camera) public isSlotTenant(walletAddress) {
        ...
        SlotTenants[walletAddress].camera==0;            
        nembweofslotfree+=1;
        if(camera==1){
            cam1+=1;
    ...
    function setDue(address walletAddress) internal {
        ...
        uint fiveMinuteIncrements = timespanMinutes / 3;  
        
        SlotTenants[walletAddress].due += (fiveMinuteIncrements+1) * 5000000000000000;
    }
    ...
    function Violation(address walletAddress) public {
        SlotTenants[walletAddress].due+=10000000000000000;
    }
