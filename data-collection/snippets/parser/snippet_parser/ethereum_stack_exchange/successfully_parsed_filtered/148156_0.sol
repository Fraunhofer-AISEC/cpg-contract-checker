...
contract MyToken {
    ...
    uint256 public constant minStakeAmount = 10000000000000000;
   ...
   function flashMintHook() internal {
    ...
    function buyTokens(uint256 tokensToBuy) public payable {
        require(msg.value > 0, "Amount of Ether sent should be greater than 0");

        uint256 totalTokens = tokensToBuy *(10**18);
        uint256 tokensLeft = initialSupply - totalTokensSold;

        require(tokensLeft >= totalTokens, "Insufficient tokens left for sale");
        uint256 costInWei = tokenPrice * totalTokens;
        require(costInWei <= msg.value, "Insufficient Ether sent");
        ...
        emit Transfer(address(this), msg.sender, totalTokens);
