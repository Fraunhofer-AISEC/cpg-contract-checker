...
class StatementHandler(lang: SolidityLanguageFrontend) :
    Handler<Statement, ParserRuleContext, SolidityLanguageFrontend>(
    ...
    init {
        map.put(SolidityParser.BlockContext::class.java) {
            handleBlock(it as SolidityParser.BlockContext)
        }
        map.put(SolidityParser.StatementContext::class.java) {
            handleStatement(it as SolidityParser.StatementContext)
        }
        map.put(SolidityParser.SimpleStatementContext::class.java) {
            handleSimpleStatement(it as SolidityParser.SimpleStatementContext)
        }
        map.put(SolidityParser.ReturnStatementContext::class.java) {
            handleReturnStatement(it as SolidityParser.ReturnStatementContext)
        }
        map.put(SolidityParser.ExpressionStatementContext::class.java) {
            handleExpressionStatement(it as SolidityParser.ExpressionStatementContext)
        }
        map.put(SolidityParser.VariableDeclarationStatementContext::class.java) {
            handleVariableDeclarationStatement(
                it as SolidityParser.VariableDeclarationStatementContext
            )
        }

        map.put(SolidityParser.IfStatementContext::class.java) {
            handleIfStatement(it as SolidityParser.IfStatementContext)
        }
        map.put(SolidityParser.TryStatementContext::class.java) {
            handleTryStatement(it as SolidityParser.TryStatementContext)
        }
        map.put(SolidityParser.WhileStatementContext::class.java) {
            handleWhileStatement(it as SolidityParser.WhileStatementContext)
        }
        map.put(SolidityParser.ForStatementContext::class.java) {
            handleForStatement(it as SolidityParser.ForStatementContext)
        }
        map.put(SolidityParser.InlineAssemblyStatementContext::class.java) {
            handleInlineAssemblyStatement(it as SolidityParser.InlineAssemblyStatementContext)
        }
        map.put(SolidityParser.DoWhileStatementContext::class.java) {
            handleDoWhileStatement(it as SolidityParser.DoWhileStatementContext)
        }
        map.put(SolidityParser.ContinueStatementContext::class.java) {
            handleContinueStatement(it as SolidityParser.ContinueStatementContext)
        }
        map.put(SolidityParser.BreakStatementContext::class.java) {
            handleBreakStatement(it as SolidityParser.BreakStatementContext)
        }
        map.put(SolidityParser.ThrowStatementContext::class.java) {
            handleThrowStatement(it as SolidityParser.ThrowStatementContext)
        }
        map.put(SolidityParser.EmitStatementContext::class.java) {
            handleEmitStatement(it as SolidityParser.EmitStatementContext)
        }
        map.put(SolidityParser.UncheckedStatementContext::class.java) {
            handleUncheckedStatement(it as SolidityParser.UncheckedStatementContext)
        }
        map.put(SolidityParser.RevertStatementContext::class.java) {
            handleRevertStatement(it as SolidityParser.RevertStatementContext)
        }
    }
    ...
    private fun handleBlock(ctx: SolidityParser.BlockContext): CompoundStatement {
        ...
        for (stmt in ctx.statement()) {
            handle(stmt)?.let { compound.addStatement(it) }
        }
    ...
    public fun handleMissingBlock(ctx: SolidityParser.SourceUnitContext): CompoundStatement {
        ...
        for (stmt in ctx.statement().union(ctx.expressionStatement())) {
            handle(stmt)?.let { compound.addStatement(it) }
        }
    ...
    private fun handleStatement(ctx: SolidityParser.StatementContext): Statement {
        ...
        return newProblemExpression(
            "Statement type not implemented for translation.",
    ...
    private fun handleExpressionStatement(
        ...
        ctx.expression()?.let {
            ...
            if (
                expression != null &&
                    expression is DeclaredReferenceExpression &&
                    expression.code == "_" &&
                    frontend.modifierStack.isNotEmpty()
            ) { // Is name correct here or do we need local name or code?
    ...
    private fun handleVariableDeclarationStatement(
        ctx: SolidityParser.VariableDeclarationStatementContext
        ...
        ctx.variableDeclarationList()?.let { list.addAll(it.variableDeclaration()) }

        ctx.variableDeclaration()?.let { list += it }
        ...
        for (variable in list) {
            ...
            val name = variable.identifier().text.trim()

            val decl =
                newVariableDeclaration(
                    name,
                    type ?: newUnknownType(),
                    frontend.getCodeFromRawNode(variable.identifier()),
                    false
                )
    ...
    private fun handleIfStatement(ctx: SolidityParser.IfStatementContext): IfStatement {
        ...
        ctx.expression()?.let { ifStatement.condition = frontend.expressionHandler.handle(it) }

        ctx.statement(0)?.let { ifStatement.thenStatement = handle(it) }

        ctx.statement(1)?.let { ifStatement.elseStatement = handle(it) }
    ...
    private fun handleWhileStatement(ctx: SolidityParser.WhileStatementContext): WhileStatement {
        ...
        ctx.expression()?.let { whileStatement.condition = frontend.expressionHandler.handle(it) }

        ctx.statement()?.let { whileStatement.statement = handle(it) }
    ...
    private fun handleForStatement(ctx: SolidityParser.ForStatementContext): ForStatement {
        ...
        ctx.simpleStatement()?.let { forStatement.initializerStatement = handle(it) }
        ...
        ctx.statement()?.let { forStatement.statement = handle(it) }
    ...
    private fun handleDoWhileStatement(ctx: SolidityParser.DoWhileStatementContext): DoStatement {
        ...
        ctx.statement()?.let { doStatement.statement = handle(it) }

        ctx.expression()?.let { doStatement.condition = frontend.expressionHandler.handle(it) }
    ...
    private fun handleContinueStatement(
        ctx: SolidityParser.ContinueStatementContext
    ): ContinueStatement {
    ...
    private fun handleInlineAssemblyStatement(
        ctx: SolidityParser.InlineAssemblyStatementContext
    ): Statement {
