...
class AddressPaddingCheck : Check() {
    ...
    override fun check(transaction: Transaction): List<PhysicalLocation> {
        ...
        val query =
            "match p=(f:FunctionDeclaration)-[:EOG|INVOKES*]->(c:CallExpression)-[:EOG|INVOKES*]->(last)<--(f)\n" +
                    "where (toUpper(c.name) in ['TRANSFER' , 'SEND']\n" +
                    "    and exists{\n" +
                    "        (f)-[r:PARAMETERS]->(param:ParamVariableDeclaration)-[:DFG*]->()<-[:ARGUMENTS]-(v)\n" +
                    "        where not exists {(f)-[rp:PARAMETERS]->() where rp.INDEX > r.INDEX}\n" +
                    "    }\n" +
                    "    or exists{\n" +
                    "        (f)-[r:PARAMETERS]->(param:ParamVariableDeclaration)-[:DFG*]->()<-[:VALUE]-(s)-[:KEY]->({value: 'value'})\n" +
                    "            where exists{(s)<--(c)} and not exists {(f)-[rp:PARAMETERS]->() where rp.INDEX > r.INDEX}\n" +
                    "        }\n" +
