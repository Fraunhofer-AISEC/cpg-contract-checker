...
RUN wget -O - https://debian.neo4j.com/neotechnology.gpg.key | apt-key add - && echo 'deb https://debian.neo4j.com stable latest' | tee -a /etc/apt/sources.list.d/neo4j.list && apt-get update && apt-get install -y neo4j && rm -rf /var/lib/apt/lists/*
