...
class DOSCheck : Check() {
    ...
    override fun check(transaction: Transaction): List<PhysicalLocation> {
        ...
        // Adding secondary query that checks if one of our state changes can be influenced by a
        // padded value
        var query =
            object {}
                .javaClass
                .getResourceAsStream("/DOSThroughExhaustion")
                ?.bufferedReader()
                ?.readText()
        ...
        query =
            object {}
                .javaClass
                .getResourceAsStream("/CallBlocksStateChangesOrCalls")
                ?.bufferedReader()
                ?.readText()
        query = configurePaths(query)
        ...
        // A query that finds if we make an external call whos failure stops other calls from
        // executing
        query =
            object {}
                .javaClass
                .getResourceAsStream("/CallBlocksFutureCall")
                ?.bufferedReader()
                ?.readText()
        query = configurePaths(query)
        ...
        query =
            object {}
                .javaClass
                .getResourceAsStream("/EmptyCollectionUsedForTransfers")
                ?.bufferedReader()
                ?.readText()
        query = configurePaths(query)
