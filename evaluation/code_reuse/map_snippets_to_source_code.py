...
SNIPPETS_DATASET = [
    ("stack_overflow", "../../data-collection/snippets/parser/snippet_parser/stack_overflow/successfully_parsed"),
    ("ethereum_stack_exchange", "../../data-collection/snippets/parser/snippet_parser/ethereum_stack_exchange/successfully_parsed")
]
...
def main():
    for dataset_name, dataset_path in SNIPPETS_DATASET:
        ...
        if not os.path.exists("ngram_"+str(NGRAM_THRESHOLD)+"_levenshtein_"+str(LEVENSHTEIN_TRESHOLD)+"/"+dataset_name+"_matches"):
            os.makedirs("ngram_"+str(NGRAM_THRESHOLD)+"_levenshtein_"+str(LEVENSHTEIN_TRESHOLD)+"/"+dataset_name+"_matches")

        analyzed_snippets = set()
        if not os.path.exists("ngram_"+str(NGRAM_THRESHOLD)+"_levenshtein_"+str(LEVENSHTEIN_TRESHOLD)+"/"+dataset_name+"_stats.csv"):
            with open("ngram_"+str(NGRAM_THRESHOLD)+"_levenshtein_"+str(LEVENSHTEIN_TRESHOLD)+"/"+dataset_name+"_stats.csv", "w") as csvfile:
            ...
            with open("ngram_"+str(NGRAM_THRESHOLD)+"_levenshtein_"+str(LEVENSHTEIN_TRESHOLD)+"/"+dataset_name+"_stats.csv", "r") as csvfile:
        ...
        for snippet in all_snippets:
            if not snippet in analyzed_snippets:
                ...
                with open("ngram_"+str(NGRAM_THRESHOLD)+"_levenshtein_"+str(LEVENSHTEIN_TRESHOLD)+"/"+dataset_name+"_matches/"+snippet.split("/")[-1].replace(".sol", "")+".csv", "w") as csvfile:
                    writer = csv.writer(csvfile)
                    ...
                    with open("ngram_"+str(NGRAM_THRESHOLD)+"_levenshtein_"+str(LEVENSHTEIN_TRESHOLD)+"/"+dataset_name+"_stats.csv", "a") as csvfile2:
